...
RelayDeck Local 是一个面向本地部署场景的模型路由与管理平台。项目将 LiteLLM 统一网关、Claude 网关和自研管理后台整合在一起，目标是解决多供应商模型接入、统一命名、路由兜底、额度监控、接口测试和运维可视化等问题。
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item 支持 Codex、OpenCode 和 Claude Code 通过统一网关调用已经配置好的模型，而不是手工切换不同供应商。
\end{enumerate}
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item 客户端接入层：由 Codex、OpenCode 和 Claude Code 通过对应协议调用统一网关。
\end{enumerate}

其请求链路可概括为：

\begin{center}
用户请求 $\rightarrow$ 客户端 $\rightarrow$ LiteLLM 或 Claude 网关 $\rightarrow$ 目标供应商 API
\end{center}
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item 测试对话：发送最小聊天请求，验证模型是否真能被客户端调用，结果更接近实际使用场景。
\end{enumerate}
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item 系统联调测试：通过客户端发起实际对话，验证统一模型入口与 fallback 是否按预期工作。
\end{enumerate}
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item 继续优化客户端模型列表，只暴露最终可用的公共模型名。
\end{enumerate}
